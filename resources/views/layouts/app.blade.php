                    ...
                    <div class="hidden sm:flex sm:space-x-4">
                        @auth
                            @if(Auth::user()->role === 'admin')
                                ...
                                <div x-data="{ openMaster: false }" class="relative">
                                    <button
                                        @click="openMaster = !openMaster"
                                        @click.outside="openMaster = false"
                                        class="border-transparent inline-flex items-center gap-1 px-1 pt-1 border-b-2 text-sm font-medium transition-colors
                                        {{ request()->routeIs('admin.users.*') || request()->routeIs('admin.kategori.*') || request()->routeIs('admin.ruangan.*')
                                            ? 'border-blue-500 text-gray-900 dark:text-white'
                                            : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200' }}">
                                        Master Data
                                        <svg class="w-3.5 h-3.5 transition-transform duration-150" :class="openMaster ? 'rotate-180' : ''"
                                    ...
                                    <div x-show="openMaster"
                                         ...
                                         class="absolute top-full left-0 mt-1 w-44 bg-white dark:bg-slate-800 rounded-lg
